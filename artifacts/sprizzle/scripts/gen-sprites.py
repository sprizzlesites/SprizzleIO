...
CONFIGS = [
    ("scroll-bg.mp4",        "sprite-desktop.webp", 1280, 720,  80),
    ("scroll-bg-mobile.mp4", "sprite-mobile.webp",   640, 360,  80),
]
N_FRAMES, COLS, ROWS = 100, 10, 10
PUBLIC = "artifacts/sprizzle/public"

for src, dst, fw, fh, quality in CONFIGS:
    ...
    cap.release()
    sheet.save(f"{PUBLIC}/{dst}", "WEBP", quality=quality, method=6)
    size_mb = os.path.getsize(f"{PUBLIC}/{dst}") / 1_000_000
